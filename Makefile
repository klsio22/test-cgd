...
## Docker stack
.PHONY: stack-start
stack-start:  ## Start local stack
	@echo "Start local Docker stack"
	@docker-compose -f docker-compose.yml up -d --build
	@echo "http://test-cgd.localhost"

.PHONY: start-stack
stack-create-site:  ## Create a new site
...
build-acceptance-servers: ## Build Acceptance Servers
	...
	@docker build backend -t collective/test-cgd-backend:acceptance -f backend/Dockerfile.acceptance
	@echo "Build acceptance frontend"
	@docker build frontend -t collective/test-cgd-frontend:acceptance -f frontend/Dockerfile

.PHONY: start-acceptance-servers
start-acceptance-servers: build-acceptance-servers ## Start Acceptance Servers
	@echo "Start acceptance backend"
	@docker run --rm -p 55001:55001 --name test-cgd-backend-acceptance -d collective/test-cgd-backend:acceptance
	@echo "Start acceptance frontend"
	@docker run --rm -p 3000:3000 --name test-cgd-frontend-acceptance --link test-cgd-backend-acceptance:backend -e RAZZLE_API_PATH=http://localhost:55001/plone -e RAZZLE_INTERNAL_API_PATH=http://backend:55001/plone -d collective/test-cgd-frontend:acceptance

.PHONY: stop-acceptance-servers
stop-acceptance-servers: ## Stop Acceptance Servers
	@echo "Stop acceptance containers"
	@docker stop test-cgd-frontend-acceptance
	@docker stop test-cgd-backend-acceptance
